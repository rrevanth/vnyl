 ...
 * 
 * Handles pagination for specific recommendation catalogs with complete context preservation.
...
/**
 * Input parameters for LoadMoreRecommendationsUseCase
 */
export interface LoadMoreRecommendationsRequest {
  ...
  /** The original media item the recommendations are based on */
...
/**
 * Result type for LoadMoreRecommendationsUseCase
 */
...
export class LoadMoreRecommendationsUseCase {
  constructor(
    private readonly recommendationsProviders: IRecommendationsProvider[],
    private readonly logger: ILoggingService
  ) {}

  /**
   * Execute the use case to load more recommendation items
   */
  async execute(request: LoadMoreRecommendationsRequest): Promise<LoadMoreRecommendationsResult> {
    const requestId = `load-more-recommendations-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
    const startTime = performance.now()
    const limit = request.limit ?? 20

    try {
      this.logger.info('Starting LoadMoreRecommendationsUseCase execution', {
        context: 'load_more_recommendations_usecase',
        requestId,
      ...
      this.logger.debug('Found recommendations provider', undefined, {
        context: 'load_more_recommendations_usecase',
        requestId,
      ...
      this.logger.debug('Successfully loaded recommendation items', undefined, {
        context: 'load_more_recommendations_usecase',
        requestId,
        providerId: provider.id,
        catalogId: request.catalogId,
      ...
      // Get the full recommendations to extract updated pagination info
      // This is necessary to get accurate pagination metadata
      const fullRecommendations = await provider.getRecommendations(
        request.originalMediaItem, 
        { page: request.page, limit }
      )
      
      // Find the matching catalog from the recommendations
      const fullCatalog = fullRecommendations.recommendations.find(
        catalog => catalog.id === request.catalogId
      )
      
      if (!fullCatalog) {
        throw new Error(`Catalog not found in recommendations: ${request.catalogId}`)
      }
      ...
      this.logger.info('LoadMoreRecommendationsUseCase execution completed', {
        context: 'load_more_recommendations_usecase',
        requestId,
        itemsLoaded: items.length,
        isLastPage,
      ...
      this.logger.error('LoadMoreRecommendationsUseCase execution failed', errorInstance, {
        context: 'load_more_recommendations_usecase',
        requestId,
        providerId: request.providerId,
        catalogId: request.catalogId,
      ...
      throw new Error(`Failed to load more recommendation items: ${errorInstance.message}`)
    }
  }
   ...
   */
  private validateRequest(request: LoadMoreRecommendationsRequest, requestId: string): void {
  ...
  /**
   * Finds a provider by its ID that supports recommendations capability
   */
  private findProviderById(providerId: string): IRecommendationsProvider | null {
    const provider = this.recommendationsProviders.find(provider => provider.id === providerId)
    
    if (!provider) {
      return null
    }

    // Verify the provider supports recommendations capability
    if (!provider.capabilities.includes(ProviderCapability.RECOMMENDATIONS)) {
      this.logger.warn('Provider does not support recommendations capability', undefined, {
        context: 'load_more_recommendations_usecase',
        providerId: provider.id,
   ...
   */
  private async ensureProviderInitialized(provider: IRecommendationsProvider, requestId: string): Promise<void> {
    try {
